...
public:
	...
	std::weak_ptr<Object> getObjectFromChildren(int id) const
	{
		...
		for (auto& ins : ins_) {		//追加待ちのオブジェクトからも探す
			ret = ins->getObjectFromChildren(id);
			if (!ret.expired()) return ret;
		}
		return ret;
	}
	...
	virtual int receiveMsg(std::weak_ptr<Object> sender, const int num) { return 0; }
	virtual int postMsg(std::weak_ptr<Object> receiver, const std::string& msg) { return receiver.lock()->receiveMsg(selfPtr(), msg); }
	virtual int postMsg(std::weak_ptr<Object> receiver, const int num) { return receiver.lock()->receiveMsg(selfPtr(), num); }

	virtual int receiveMsg(Object* sender, const std::string& msg) { return 0; }
	virtual int receiveMsg(Object* sender, const int num) { return 0; }
	virtual int postMsg(Object* receiver, const std::string& msg) { return receiver->receiveMsg(this, msg); }
	virtual int postMsg(Object* receiver, const int num) { return receiver->receiveMsg(this, num); }
